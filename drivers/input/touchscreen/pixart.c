...
static int pixart_check_and_load_fw(struct pixart_data *ts, int cnt)
{
	int i, len, retry, err;
	uint8_t val;
	uint8_t *pfw;
	...
	if(val >= PIXART_LATEST_FW && cnt == FIRST){
...
static int pixart_init_panel(struct pixart_data *ts)
{
	if (pixart_check_and_load_fw(ts, FIRST)){
...
force_dl:
	...
	if(pixart_reset_and_wait(ts)){
		pr_err("%s: Fail Reset and Wait\n",__func__);
		return -1;
	}

	if (pixart_check_and_load_fw(ts, SECOND)){
		pr_err("%s: Fail Load FW :SECOND\n",__func__);
		return -1;
	}

	if (pixart_check_and_load_ds_map(ts)){
		pr_err("%s: Fail Drive_Sense Map DL :SECOND\n",__func__);
		return -1;
	}

	if (pixart_set_config(ts)){
		pr_err("%s: Fail Set config :SECOND\n",__func__);
		return -1;
	}
...
static int pixart_reset_process(struct pixart_data *ts)
{
	...
	err = pixart_check_and_load_fw(ts, FIRST);
	if (err){
		pr_err("%s: Fail Load FW\n",__func__);
		goto done;
	}
