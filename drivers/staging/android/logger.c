...
static unsigned char check_smem_crash_system( ram_log_info_type *plog_info )
{
	...
	if ( (0 == strncmp((const char *)plog_info->crash_system, CRASH_SYSTEM_KERNEL  , strlen(CRASH_SYSTEM_KERNEL ))) ||
	     ...
	     (0 == strncmp((const char *)plog_info->crash_system, CRASH_SYSTEM_UNKNOWN , strlen(CRASH_SYSTEM_UNKNOWN))) )
	{
		ret = 1;
	}
...
static void clear_smem_crash_system(void)
{
	...
	memset( &plog_info->crash_system[0], 0x00, CRASH_CODE_SIZE );
	mb();
}
...
static unsigned char check_smem_crash_kind( ram_log_info_type *plog_info )
{
	...
	if ( (0 == strncmp((const char *)plog_info->crash_kind, CRASH_KIND_PANIC      , strlen(CRASH_KIND_PANIC     ))) ||
	     ...
	     (0 == strncmp((const char *)plog_info->crash_kind, CRASH_KIND_UNKNOWN    , strlen(CRASH_KIND_UNKNOWN   ))) )
	{
		ret = 1;
	}
...
static void clear_smem_crash_kind(void)
{
	...
	memset( &plog_info->crash_kind[0], 0x00, CRASH_CODE_SIZE );
	mb();
}
...
void set_kcj_pet_time(void)
{
	...
	get_crash_time( &pPanicInfo->pet_time[0], PET_TIME_SIZE );
}
...
static void clear_kcjlog_crash_info(void)
{
	ram_log_info_type *pKcjLogInfo;

	pKcjLogInfo   = (ram_log_info_type *)ADDR_CONTROL_INFO;

	memset( &pKcjLogInfo->crash_system[0], 0x00, CRASH_CODE_SIZE );
	memset( &pKcjLogInfo->crash_kind[0]  , 0x00, CRASH_CODE_SIZE );
	memset( &pKcjLogInfo->crash_time[0]  , 0x00, CRASH_TIME_SIZE );

	memset( &pKcjLogInfo->crash_info_data[0], 0x00, CRASH_INFO_DATA_SIZE );
	memset( &pKcjLogInfo->panic_info_data[0], 0x00, CRASH_INFO_DATA_SIZE );
}
