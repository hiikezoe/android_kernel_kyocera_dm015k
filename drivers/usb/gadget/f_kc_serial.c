...
static struct usb_string kc_gser_string_defs[] = {
	[0].s = "DM015K",
	{  } /* end of list */
};

static struct usb_gadget_strings kc_gser_string_table = {
	.language =		0x0409,	/* en-us */
	.strings =		kc_gser_string_defs,
};

static struct usb_gadget_strings *kc_gser_strings[] = {
	&kc_gser_string_table,
	NULL,
};
...
static int kc_gport_setup(struct usb_configuration *c)
{
	...
	if (kc_no_sdio_ports)
		ret = gsdio_setup(c->cdev->gadget, kc_no_sdio_ports);
	if (kc_no_smd_ports)
...
int kc_gser_bind_config(struct usb_configuration *c, u8 port_num)
{
	...
	if (kc_gser_string_defs[0].id == 0) {
		status = usb_string_id(c->cdev);
		if (status < 0)
			return status;
		kc_gser_string_defs[0].id = status;
		kc_gser_control_interface_desc.iInterface = status;
	}
	...
	gser->port.func.strings = kc_gser_strings;
